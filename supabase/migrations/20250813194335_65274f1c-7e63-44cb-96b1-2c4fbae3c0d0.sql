...
CREATE POLICY "Usernames are viewable by everyone" ON public.users
FOR SELECT 
USING (true)
WITH CHECK (false);

-- But we need to create a view or function to only expose usernames
-- Let's create a new policy that only allows access to specific columns
DROP POLICY "Usernames are viewable by everyone" ON public.users;

-- Create a policy that allows public access but we'll handle column restrictions in the app layer
